...
//—————————— Mica singing (round with other ghosts) ——————————
function OnMicaSong@Start
{
	//TODO - would like a nonoverlap option here
	MicaSongChoice = "test";
	MicaSongLength = Reflection.Get("MicaSong@{MicaSongChoice}").length;
	MicaSongPos = 0;
	
	output = "";
	
	//Send command to all open ghosts to see who responds
	local participants = [];
	for (local i = 0; i < OpenGhosts.length; i++)
	{
		local ghost = OpenGhosts[i];
		participants.Add(ghost);
		output += `\![raiseother,"{ghost}",OnMicaSong.Join,"{MicaSongChoice}"]`;
	}
	
	//Shuffle the array of potential participants so it's not based on load order
	...
	output += MicaSongStartTalk() + "\![raise,OnMicaSong@Control,0,start]";
	return output;
}

//Ghosts that are not participating in the round
function OnRaiseOtherFailure
{
	for (local i = 0; i < MicaSongParticipants.length; i++)
	{
		...
		{
			MicaSongNonParticipants.Add(MicaSongParticipants[i]);
...
talk MicaSongStartTalk
{
	\s[2]Sure sure, let's sing~!
}

function OnMicaSong@Control
{
	local lyrics = "";
	lyrics = Reflection.Get("MicaSong@{MicaSongChoice}")();
	
	local output = "";
	local currentpos = Shiori.Reference[0].ToNumber();
	if (Shiori.Reference[1] != "end") //She hasn't finished singing all the lines yet
	{
		output += `\![embed,OnMicaSong@Lyric,"{MicaSongChoice}",{MicaSongParticipants.length},{lyrics.length},{relativepos},"{lyrics[currentpos]}"]`;
		output += "\_w[2000]"; //Note to self: don't switch this to \__w because then users with slow text settings will have a worse display
	}
	else //Waiting on other ghosts to finish up
	{
		output += "\C\_w[3000]";
	}
	currentpos++;
	
	local participantsdone = 1;
	for (local i = 0; i < MicaSongParticipants.length; i++)
	{
		local relativepos = currentpos - i - 1;
		if (!(relativepos < 0 || relativepos >= lyrics.length))
		{
			participantsdone = 0;
			output += `\![raiseother,"{MicaSongParticipants[i]}",OnMicaSong.Lyric,"{MicaSongChoice}",{MicaSongParticipants.length},{lyrics.length},{relativepos},"{lyrics[relativepos]}"]`;
		}	
	}
	
	//Send out the start of song event to non-participants
	if (Shiori.Reference[1] == "start")
	{
		for (local i = 0; i < MicaSongNonParticipants.length; i++)
		{
			output += `\![raiseother,"{MicaSongNonParticipants[i]}",OnMicaSong.Listen,"{MicaSongChoice}",{MicaSongParticipants.length},{lyrics.length}]`;
		}
	}
	
	if (currentpos >= lyrics.length) //Reached the end of the song
	{
		if (!participantsdone)
		{
			output += "\![raise,OnMicaSong@Control,{currentpos},end]";
		}
		else
		{
			return OnMicaSong@Finish();
		}
	}
	else
	{
		output += "\![raise,OnMicaSong@Control,{currentpos}]";
	}
	return output;
}

function OnMicaSong@Finish
{
	local command = "";
	for (local i = 0; i < MicaSongParticipants.length; i++)
	{
		command += `\![raiseother,{MicaSongParticipants[i]},OnMicaSong.End,"{MicaSongChoice}",{MicaSongParticipants.length},{lyrics.length}]`;
	}
	
	for (local i = 0; i < MicaSongNonParticipants.length; i++)
	{
		command += `\![raiseother,{MicaSongNonParticipants[i]},OnMicaSong.BEIMPRESSED,"{MicaSongChoice}",{MicaSongParticipants.length},{lyrics.length}]`;
	}
	return "\_w[3000]" + command + OnMicaSong@End();
}
...
talk OnMicaSong@Lyric
{
	{Shiori.Reference[4]}~
}

function MicaSong@test
{
	return [
